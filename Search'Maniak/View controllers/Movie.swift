...
class MovieViewController: UIViewController, UISearchBarDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if (data.songs.count == 0 && searchBar.text != "") {
            self.tableView.setEmptyMessage("Désolé! Aucun film n'a été trouvé.")
        } else {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        if !text.isEmpty{
            data=api.search(keywords: text, mediaType: "movie")
        }
