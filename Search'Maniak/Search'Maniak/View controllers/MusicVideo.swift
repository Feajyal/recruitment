...
class MusicVideoViewController: UIViewController, UISearchBarDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if (data.songs.count == 0 && searchBar.text != "") {
            self.tableView.setEmptyMessage("Désolé! Aucun clip musical n'a été trouvé.")
            self.message.text = ""
        } else {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        if !text.isEmpty{
            data=api.search(keywords: text, mediaType: "musicVideo")
        }
